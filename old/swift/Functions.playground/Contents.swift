...
personalGreeting(name: "Raghu")

func divide(_ numerator: Int, by denominator: Int) -> Int {
    return numerator / denominator
}

let result = divide(20, by: 2)


//**************Closures***********/

let volunteerCounts = [1, 3, 40, 32, 2, 53, 77, 13]
let volunteerNames = ["Raghu", "Raja", "Dhinesh"]

let sortingFunc = { (name1: String, name2: String) -> Bool in
    if name1 == "Raghu" {
        return true
    } else if name2 == "Raghu" {
        return false
...
let raghuFirst = volunteerNames.sorted(by: sortingFunc)

let numberSorted = volunteerCounts.sorted(by: {(number1: Int, number2: Int) -> Bool in
    if number1 == 32 {
        return true
    } else if number2 == 32 {
        return false
...
let sayHellloo = { (name: String) -> String in
    "Hi \(name)"
}

let resu = sayHellloo("Raghu")
...
var team = ["Raghu", "Raja", "Guna", "Dhinesh", "Vinoth", "Dhina"]
let sortedTeam = team.sorted { name1, name2 in
    if name1 == "Raghu" {
        return true
    } else if name2 == "Raghu" {
        return false
...
let sorting = team.sorted {
    return $0 > $1
}

let moreSorting = team.sorted { $0 > $1}
print(moreSorting)

let rFilter = team.filter { name in
    return name.hasPrefix("R")
}
...
func multiClosures(first: () -> Void, second: () -> Void, third: () -> Void) {
    print("First work")
    first()
    print("Second work")
    second()
...
multiClosures {
    print("Doing the first work")
} second: {
    print("Doing the second work")
} third: {
    print("Doing the third work")
}
...
//func makeTeam(names: [String], generator: (String) -> Bool) -> [String] {
//    var members = [String]()
//    for name in names {
//        let newName = generator(name)
//        members.append(newName)
//    }
//    return members
//}
//
//let newTeam = makeTeam(names: names) { name in
//    name.hasPrefix("R")
//}


let zNames = names.filter { name in
    name.hasPrefix("Z")
}


let secondVolunteerCounts = [1,3,40,32,2,53,77,13]
func isAscending(first: Int, second: Int) -> Bool {
    first > second
}
secondVolunteerCounts.sorted(by: isAscending)


//Trailing closure syntax
let sortedVolunteers = secondVolunteerCounts.sorted { i, j in
    i < j
}

//Positional Variable Names
let volunteersSorted = secondVolunteerCounts.sorted { $0 < $1 }
print(volunteersSorted)


func format(numbers: [Int], using formatter: (Int) -> String) -> [String] {
    var result = [String]()
...
let formattedNumbers = format(numbers: secondVolunteerCounts) { i in
    "\(i) is part of the volunteer group"
}
